---
title: 2025-02-17 Waku Weekly
tags:
    - waku-updates
date: 2025-02-17
---

<!-- Please Add the team weekly highlights here, they should remain COMMENTED -->

<!-- 

- research:
- nwaku:
- js-waku:
- chat: 

-->

## [Store Service Upgrade](https://github.com/waku-org/pm/milestone/27)

- [DOS protection for req-res protocols and metrics](https://github.com/waku-org/pm/issues/66)
  - achieved:
  - next:
  - blockers:

## [Direct Message Reliability](https://github.com/waku-org/pm/milestone/28)

- [Reliability Protocol for Resource-Restricted Clients](https://github.com/waku-org/pm/issues/186)
  - achieved: [research] new specification for p2p reliability [open for review](https://github.com/waku-org/specs/pull/52)
  - next:
    - [research] review and merge p2p reliability spec
    - [nwaku] nwaku implementation of the new lightpush protocol version ready https://github.com/waku-org/nwaku/issues/2722
      - [lightpush v2](https://github.com/waku-org/nwaku/pull/3279)
      - [enhance error propagation](https://github.com/vacp2p/nim-libp2p/pull/1243)
  - blockers:

- [PostgreSQL Optimisation phase 1](https://github.com/waku-org/pm/issues/260)
  - achieved:
  - next:
  - blockers:

## [End-to-end reliability protocol](https://github.com/waku-org/pm/milestone/29)

- [End-to-end reliability protocol - Status integration](https://github.com/waku-org/pm/issues/194)
  - achieved: [research] started experimenting with status-go integration
  - next: [research] continue status-go integration, collaboration with js-waku
  - blockers:

## [Scale up number of Communities](https://github.com/waku-org/pm/milestone/32)

- [Usage of Rendezvous](https://github.com/waku-org/pm/issues/199)
  - achieved:
  - next:
  - blockers:

- [DoS protection for req-res protocols and metrics (go-waku as service node)](https://github.com/waku-org/pm/issues/200)
  - achieved:
  - next:
    - [nwaku] [Measure impact between protocols](https://github.com/waku-org/nwaku/issues/3060)
      - Measure cross performance of req-res protocols in specific usage conditions.
      - [non-relay protocols cross performance measure metrics](https://github.com/waku-org/nwaku/pull/3299)
  - blockers:

## [RLN Mainnet](https://github.com/waku-org/pm/milestone/34)

- [Implement RLN smart contract for paid, multilevel memberships](https://github.com/waku-org/pm/issues/228)
  - achieved: [research] Draft PR for contract address changes in nwaku
  - next: [research] update RLN contract tests in nwaku
  - blockers:

- [RLN contract revision and audit](https://github.com/waku-org/pm/issues/257)
  - achieved:
  - next:
  - blockers:

- [Testnet L2 with on-chain tree](https://github.com/waku-org/pm/issues/286)
  - achieved:
  - next:
    - [nwaku] [deprecate tree sync strategy](https://github.com/waku-org/nwaku/issues/2924)
  - blockers:

- [RLNv2 Web management interface](https://github.com/waku-org/pm/issues/281)
  - achieved: [js-waku] [moved js-rln library to main monorepo](https://github.com/waku-org/js-rln/issues/69)
  - next: [js-waku] [migrate to RLNv2 contract](https://github.com/waku-org/js-waku/issues/2242)
  - blockers:

- [Deploy RLN smart contract to a L2 mainnet](https://github.com/waku-org/pm/issues/258)
  - achieved:
  - next:
  - blockers:

## [Nwaku in Status Desktop (relay mode)](https://github.com/waku-org/pm/milestone/33)

- [Nwaku on Windows](https://github.com/waku-org/pm/issues/239)
  - achieved:
  - next:
    - [nwaku] [CI Integration of windows build system](https://github.com/waku-org/nwaku/issues/2473)
  - blockers:

- [Nwaku in Status Desktop](https://github.com/waku-org/pm/issues/203)
  - achieved:
    - [nwaku] [libwaku store request parsing](https://github.com/waku-org/nwaku/pull/3294)
    - [nwaku] [support parallel libwaku requests](https://github.com/waku-org/nwaku/pull/3296)
    - [nwaku] [hash to bytes conversion and increase channel size](https://github.com/waku-org/waku-go-bindings/pull/32)
    - [nwaku] [add makefile targets](https://github.com/waku-org/waku-go-bindings/pull/34)
    - [nwaku] Improving private branches for `status-desktop` and `status-go` with nwaku, and getting `status-desktop` to run properly with nwaku as a backend in Mac
  - next:
  - blockers:

- [MessagingAPI: Rust SDK](https://github.com/waku-org/pm/issues/289)
  - achieved:
  - next:
  - blockers:

## [Incentivise running a Waku infrastructure node](https://github.com/waku-org/pm/milestone/35)

- [Service Incentivisation - Pay for RLN provision PoC](https://github.com/waku-org/pm/issues/245)
  - achieved: [research] started work on local client-side reputation for the POC
  - next: [research] continue work on local client-side reputation for the POC
  - blockers:

- [Service Incentivisation - Service incentivisation roadmap & spec](https://github.com/waku-org/pm/issues/246)
  - achieved:
  - next:
  - blockers:

## [Foundation for Communities Optimization](https://github.com/waku-org/pm/milestone/31)

- [Define and implement sharding for Communities messages](https://github.com/waku-org/pm/issues/266)
  - achieved:
  - next:
  - blockers:

- [Review usage of content topics in Status Communities protocol](https://github.com/waku-org/pm/issues/268)
  - achieved:
  - next:
  - blockers:

## [Hardening and scaling foundations for private chats](https://github.com/waku-org/pm/milestone/40)

- [Isolate private chat traffic](https://github.com/waku-org/pm/issues/278)
  - achieved:
  - next:
  - blockers:

- [Specify private chat protocol](https://github.com/waku-org/pm/issues/285)
  - achieved:
    - [chat] content topics usaga in one-to-one chats [ref](https://www.notion.so/Content-topics-in-one-to-one-messages-18f8f96fb65c8019b595c707b7bf20a0)
  - next:
    - [chat] continue documenting current chat protocol
  - blockers:

- [Private chat rate limit PoC](https://github.com/waku-org/pm/issues/277)
  - achieved:
  - next:
  - blockers:

## [Upgrade Waku for the Web](https://github.com/waku-org/pm/milestone/43)

- [Scalable Data Sync in browser](https://github.com/waku-org/pm/issues/280)
  - achieved:
  - next:
    - [js-waku] [implement logic for tracking sent messages](https://github.com/waku-org/js-waku/issues/2229)
    - [js-waku] [implement logic for tracking received messages](https://github.com/waku-org/js-waku/issues/2230)
    - [js-waku] [support converting bloom filter between bytes and arrays of bigint](https://github.com/waku-org/js-waku/issues/2258)
  - blockers:

- [Improved browser bootstrap](https://github.com/waku-org/pm/issues/290)
  - achieved:
  - next:
  - blockers:

- [Browser simulations](https://github.com/waku-org/pm/issues/279)
  - achieved:
  - next:
  - blockers:

## [Logos Web apps](https://github.com/waku-org/pm/milestone/42)

- [Reliable Qaku & library](https://github.com/waku-org/pm/issues/287)
  - achieved:
    - [chat] FURPS planning
  - next:
    - [chat] start extracting the Quaku library code
  - blockers:

## [Explore Peer Discovery Gap](https://github.com/waku-org/pm/milestone/44)

- [Mixnet PoC](https://github.com/waku-org/pm/issues/291)
  - achieved: [research] a basic lightpush request only POC by using waku nodes as mix nodes with hardcoded configs
  - next: [research] draft waku mix spec
  - blockers

## [Nwaku in Status Mobile MVP](https://github.com/waku-org/pm/milestone/39)

- [Status mobile build with nwaku MVP](https://github.com/waku-org/pm/issues/276)
  - achieved:
  - next:
  - blockers

## [Debugging Tools](https://github.com/waku-org/pm/milestone/38)

- [Status Local Metrics](https://github.com/waku-org/pm/issues/273)
  - achieved:
    - [chat] review and discuss the telemetry replacement PR, [ref](https://github.com/status-im/status-go/pull/6256)
    - [chat] more metrics design, [ref](https://forum.vac.dev/t/the-future-of-status-telemetry/405/1)
  - next:
    - [chat] get pending metrics PRs merged
  - blockers:

- [Local Metrics Dashboard](https://github.com/waku-org/pm/issues/274)
  - achieved:
  - next:
  - blockers:

## [Messaging API](https://github.com/waku-org/pm/milestone/41)

- [Define Messaging API](https://github.com/waku-org/pm/issues/282)
  - achieved:
  - next:
    - [js-waku] publish rfc covering event based API and basic capabilities
  - blockers:

- [Messaging API: js-waku](https://github.com/waku-org/pm/issues/283)
  - achieved:
  - next:
    - [js-waku] [enable background retries and upgrade to light push v2](https://github.com/waku-org/js-waku/issues/2054)
  - blockers:

## [Maintenance H1 2025](https://github.com/waku-org/pm/issues/275)
  - achieved:
    - [nwaku] [better proof handling in REST](https://github.com/waku-org/nwaku/pull/3286)
    - [nwaku] [add waku-rlnv2-contract as vendor dependency](https://github.com/waku-org/nwaku/pull/3289)
  - next:
    - [mwaku] [enhance metrics usage](https://github.com/waku-org/nwaku/issues/3282)
    - [nwaku] [Prepare release v0.35](https://github.com/waku-org/nwaku/issues/3251)
    - [nwaku] js-waku - nwaku interoperability test stabilization
    - [js-waku] [js-waku interoperability investigations](https://github.com/waku-org/js-waku/issues/2176)
  - blockers:

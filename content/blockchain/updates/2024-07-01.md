...
- Started writing the new mixnet [executable specs](https://github.com/logos-blockchain/logos-blockchain-specs/pull/98). Completed: Global transmission rate, Peering degree limit, Noise per connection, Gossip-based Sphinx routing. Still to do: Time mixing.
...
- WIP: First iteration of the PoC can be found [here](https://github.com/logos-blockchain/logos-blockchain-specs/pull/99). That PR already runs a libp2p inside a very basic DA node. It is important to mention that the very limited py-libp2p is not great software. The last official release is from 2020, and it had to be cloned from GitHub to make it work at all. Then, there was an issue with its internals: it's highly opinionated. Tried to reuse existing py code from `mixnet` by using `asyncio` but py-libp2p would refuse to work with it due to the lib's internal setup with `trio`. It is also important to mention that this is an exploration of the libraries; the team is yet to decide whether libp2p will be used at all.

### `Development`

- Finishing FK20 Rust [implementation](https://github.com/logos-blockchain/logos-blockchain/pull/663).
- Included FK20 in DA encoder ([PR](https://github.com/logos-blockchain/logos-blockchain/pull/664)).
- Added Rayon parallelization to DA encoder ([PR](https://github.com/logos-blockchain/logos-blockchain/pull/665)).
- Implemented cache for Toeplitz part-1 and integrated it into the encoder ([PR](https://github.com/logos-blockchain/logos-blockchain/pull/667)).
- Added benchmarks and missing parallelization features ([PR](https://github.com/logos-blockchain/logos-blockchain/pull/669)).
- DA subnetwork assignment algorithm: Implemented re-hashing algorithm and initial experiments. Tried to implement and improve a few variations of the algorithm, but none of them were better than the original. Ran a few [experiments](https://github.com/logos-blockchain/logos-blockchain-specs/tree/da-subnetwork-distribution) (results and ideas will be shared later on).
- Wrapped up DA HTTP endpoints for KZGRS backend [PR](https://github.com/logos-blockchain/logos-blockchain/pull/670). Pushed changes related to DA HTTP endpoints. They are generic and should be reused with the updated KZGRS protocol and dispersal as a black box.
...
- WIP: continuing with the PoC, understanding whether Risc0 is a viable option ([progress](https://github.com/logos-blockchain/logos-blockchain-specs/tree/cl/executable-spec)).
- Integration of Risc0 zone PoC into CL note and partial tx model: we have the zone POC hooked into the CL UTXO model, but proof times are not where we want them. We are currently at 8 minutes to generate a proof, almost all of that is coming from the Pedersen commitment to the note value. All the work is being done in this [PR](https://github.com/logos-blockchain/logos-blockchain-specs/pull/93).
...
- Testnet updates from the `testnet` branch now (deployments happen from the `testnet` branch) - [Nomos PR](https://github.com/logos-blockchain/logos-blockchain/pull/668), [Infra PR](https://github.com/status-im/infra-misc/pull/294). This is done to have more control over what runs in the testnet.
- Enabling/Disabling tracing using feature flags or config at runtime - [PR](https://github.com/logos-blockchain/logos-blockchain/pull/666). Added the ability to build the node without tracing, or toggle tracing via config if built with this feature.
